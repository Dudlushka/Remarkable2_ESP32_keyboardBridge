...
uint8_t KeyboardReportParser::OemToAscii(uint8_t mod, uint8_t key)
{
  ...
  {
    // Upper case letters
    if ((bmCapsLock == 0 && shift) || (bmCapsLock == 1 && shift == 0)) return (key - 4 + 'A');
    // Lower case letters
    else return (key - 4 + 'a');
  }
  ...
  {
    if (shift)  return ((uint8_t)pgm_read_byte(&getNumKeys()[key - 0x1e]));
  ...
  {
    if(bmNumLock == 1) return (key - 0x59 + '1');
  } 
  else if(VALUE_WITHIN(key, 0x2d, 0x38)) return ((shift) ? (uint8_t) pgm_read_byte(&getSymKeysUp()[key - 0x2d]) : (uint8_t)pgm_read_byte(&getSymKeysLo()[key - 0x2d]));
  else if(VALUE_WITHIN(key, 0x54, 0x58)) return (uint8_t) pgm_read_byte(&getPadKeys()[key - 0x54]);
  else {
    switch(key) {
