...
#define CHARBUF_LEN 8

//--------------------------------------------------------------------------
//inherited class to implement some of the virtual functions of the parent
//--------------------------------------------------------------------------
class KbdRptParser : public KeyboardReportParser
{
  protected:
    ...
    int SearchForSpecialLayout(uint8_t mod, uint8_t key) ;

    void SpecChar_enter();
    int  SpecChar_process(uint8_t key);
    int  SpecChar_release();

     void PrintAllChars();
     uint8_t intToAscii(uint8_t number) ;

    bool SpecCharMode = false;
    char SpecCharBuff[CHARBUF_LEN];
    uint8_t SpecCharBuff_i;
   

    int16_t specCharOffset = 0x20;
    
    uint32_t QTmodifier = 0;

};

extern KbdRptParser kbparser;
